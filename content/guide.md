Talkql lets you ask questions about your database in plain English — no SQL required. This guide walks through how to get the best results, how to keep your data safe, and the limits worth knowing before you rely on it.

## Getting started

You have two ways in:

- **Playground** — try Talkql instantly against a hosted sample database. No login, no key, no setup. Great for seeing how it works.
...
Talkql is **bring-your-own-key (BYOK)**. In a project's settings you choose:
...
The clearer your question, the better the query Talkql writes:
...
- **Visualization** — describe what you want to see; Talkql picks the chart type and fills it with your data.
...
- **Read-only by design.** Every query Talkql generates is read-only — it cannot insert, update, or delete your data.
...
A few things to understand before you depend on Talkql:
...
- **No cross-browser or cross-device access.** Open Talkql in a different browser, profile, or device and your projects won't be there. Clearing your browser data erases them permanently.
...
- **Results are capped.** Queries return a limited number of rows, so for large datasets Talkql prefers summaries and aggregates over dumping raw rows.
